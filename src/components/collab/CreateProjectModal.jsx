...
import { createProject } from "@/components/collab/collabService";

export default function CreateProjectModal({ open, onClose, currentUser, users, onCreated }) {
  const [form, setForm] = useState({
    title: "", description: "",
    priority: "media", status: "ativo",
    start_date: "", due_date: "",
    responsible_email: "",
    is_private: true, color_tag: "#4338CA", summary_required: false,
  });
  ...
  const filteredUsers = users.filter(u => {
    ...
    return getName(u).toLowerCase().includes(responsibleSearch.toLowerCase());
  });
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const project = await createProject(form, currentUser.email);
      onCreated(project);
      onClose();
      resetForm();
    } catch (e) {
      console.error("CreateProjectModal:", e);
    }
    setLoading(false);
  };
  ...
  const resetForm = () => {
    setForm({
      ...
      priority: "media", status: "ativo",
      start_date: "", due_date: "",
      responsible_email: "", is_private: true,
      color_tag: "#4338CA", summary_required: false,
    });
    setResponsibleSearch("");
    setSelectedResponsible(null);
  };
