...
export default function PollMessage({ message, currentUser, isOwn, allUsers }) {
  ...
  const hasVotedFor = (optionId) => (votes[optionId] || []).includes(currentUser?.email);
    ...
    <div className="space-y-3" style={{ minWidth: "min(240px, 60vw)" }}>
          ...
          return (
            <button
              ...
              className={`relative w-full text-left rounded-lg overflow-hidden border text-sm py-2 px-3 transition-all ${
                voted
                  ? isOwn ? "border-white/50 bg-white/15" : "border-primary/60 bg-primary/10"
                  : isOwn ? "border-white/20 bg-white/5 hover:bg-white/10" : "border-border bg-background/50 hover:bg-accent"
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        >
          ...
          {closed
            ? <><Lock className="w-3 h-3" /> Encerrada · {totalVotes} voto{totalVotes !== 1 ? "s" : ""}</>
